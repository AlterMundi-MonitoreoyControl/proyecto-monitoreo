...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Accede a http://[IP]/actual o http://[IP]/config");
    Serial.print("STA IP: ");
...
void loop() {
  ...
  if (!enModoLocal && WiFi.status() != WL_CONNECTED) {
    ...
    Serial.println("Accede a: http://192.168.4.1/actual o lo mismo para /config");
    enModoLocal = true;
  } else if (enModoLocal && WiFi.status() == WL_CONNECTED) {
    Serial.println("WiFi conectado. Cambiando a modo no local.");
    Serial.println("Accede a: http://192.168.4.1/actual o http://[IP]/actual , lo mismo para /config");
    Serial.print("STA IP: ");
    Serial.println(WiFi.localIP());    
    enModoLocal = false;
  }
